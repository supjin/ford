 ...
 * <p>Company: Artopcq </P>
 * @author Dana
 * @date 2017年6月17日 上午8:53:01
 * @version V1.0
 *
 */
public class Pagination extends SimplePage {

	private static final long serialVersionUID = 4302142048956366488L;
	private List<?> list;
	...
	public void pageView(String url, String params) {
		...
		if (this.pageNo != 1) {
			...
			this.pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + (this.pageNo - 1) + "'\"><font size=2>上页</font></a>");
		} else {
			this.pageView.add("<font size=2>首页</font>");
			this.pageView.add("<font size=2>上页</font>");
		}
		...
		if (this.pageNo != getTotalPage()) {
			this.pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + (this.pageNo + 1) + "'\"><font size=2>下页</font></a>");
			this.pageView.add("<a href=\"javascript:void(0);\" onclick=\"javascript:window.location.href='" + url + "?"
					+ params + "&pageNo=" + getTotalPage() + "'\"><font size=2>尾页</font></a>");
		} else {
			this.pageView.add("<font size=2>下页</font>");
			this.pageView.add("<font size=2>尾页</font>");
		}
		this.pageView.add("<font size=2>共" + getTotalPage()+"页</font>");
