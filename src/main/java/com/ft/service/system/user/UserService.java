package com.ft.service.system.user;
...
public class UserService {
	...
	*/
	...
	/*
	*用户列表(根据用户id查询 部门 职位 姓名)
	*/
	public PageData listByuser(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserXMapper.listByuser", pd);
	}
	/*
	*用户列表(全部包括admin)
	*/
		public List<PageData> listAllUsers(PageData pd)throws Exception{
		return (List<PageData>) dao.findForList("UserXMapper.listAllUsers", pd);
	}
	
	/*
	*用户列表(根据公司id 和 部门id)
	*/
	public List<PageData> listAllUserByDept(PageData pd)throws Exception{
		return (List<PageData>) dao.findForList("UserXMapper.listAllUserByDept", pd);
	}
