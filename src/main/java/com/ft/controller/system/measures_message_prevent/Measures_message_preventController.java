package com.ft.controller.system.measures_message_prevent;
...
/** 
 * 类名称：Measures_message_preventController
 * 创建人：lichao 
 * 创建时间：2017-11-15
 */
@Controller
@RequestMapping(value="/measures_message_prevent")
public class Measures_message_preventController extends BaseController {
	
	String menuUrl = "measures_message_prevent/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Measures_message_prevent");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("MEASURES_MESSAGE_PREVENT_ID", this.get32UUID());	//主键
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, "删除Measures_message_prevent");
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改Measures_message_prevent");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, "列表Measures_message_prevent");
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = measures_message_preventService.list(page);	//列出Measures_message_prevent列表
			mv.setViewName("system/measures_message_prevent/measures_message_prevent_list");
	...
	public ModelAndView goAdd() throws Exception{
		logBefore(logger, "去新增Measures_message_prevent页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			...
			mv.setViewName("system/measures_message_prevent/measures_message_prevent_edit");
	...
	public ModelAndView goEdit() throws Exception{
		logBefore(logger, "去修改Measures_message_prevent页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd.put("TYPE", "efectentType");
			List<PageData> arryList =dictService.getAll(pd);
			pd = measures_message_preventService.findById(pd);	//根据ID读取
			mv.setViewName("system/measures_message_prevent/measures_message_prevent_edit");
	...
	public Object deleteAll() throws Exception {
		logBefore(logger, "批量删除Measures_message_prevent");
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				measures_message_preventService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, "导出Measures_message_prevent到excel");
		...
		try{
			...
			List<PageData> varOList = measures_message_preventService.listAll(pd);
