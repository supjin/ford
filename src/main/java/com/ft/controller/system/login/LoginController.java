package com.ft.controller.system.login;
...
import com.ft.entity.system.Menu;
...
import com.ft.util.RightsHelper;
...
public class LoginController extends BaseController {
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					if(pd != null){
						...
						user.setCOMPANY_ID(pd.getString("COMPANY_ID"));
						session.setAttribute(Const.SESSION_USER, user);
						session.removeAttribute(Const.SESSION_SECURITY_CODE);
						session.setAttribute(Const.SESSION_USERID, user.getUSER_ID());
						//shiro加入身份验证
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu) throws Exception{
		...
		try{
			...
			if (user != null) {
				...
				mv.setViewName("pc/login");//session失效后跳转登录页面
			}
	...
	public Map<String, String> getUQX(Session session){
		...
		try {
			...
			//通过角色ID查询角色对象 ，角色对象包含了对数据的增删改查的权限
			pd = roleService.findObjectById(pd);																
			//根据角色id去查询SYS_GL_QX表中的权限
	...
	public static void main(String[] args) {
		String passwd = new SimpleHash("SHA-1", "重庆浪尖渝力科技有限公司测试", "123456").toString();
