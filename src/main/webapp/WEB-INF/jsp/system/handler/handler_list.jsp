    ...
    <div class="oiswen_titop">
        <div class="oiswen_titople">
            ...
            <div class="oiswen_titopleb">
                <a href="javascript:;"><span class="daohangbefor glyphicon glyphicon-home"></span>HOME</a><span>/</span><a href="javascript:;">系统工具</a><span>/</span><a href="javascript:;">操作者管理</a>
            </div>
        </div>
    </div>
    <form action="${ctx }/handler/list" name="souForm" id="souForm">
	<!--主体--->
	<div class="content">
		<!--导出-->
	    ...
	    <table cellpadding="0" cellspacing="0" class="lins_table">
			...
			<c:forEach items="${varList}" var="var" varStatus="vs">
	    		<tr>
	    			<td><input class="pull-right" type="checkbox" name='ids' value="${var.HANDLER_ID}" />
	    			<input type="hidden" name="HANDLER_ID" value="${var.HANDLER_ID }">
	    			</td>
	    			<td>${vs.index+1}</td>
	    			...
	    			<td>
	    				<div class="cuozuo_box clearfix">
			    			<a href="javascript:;" class="equi_bianji" onclick="updatePROCEDUREMANAGEMENT('${var.HANDLER_ID }')"></a>
...
<script>
	...
	function processdureAdd(){
		window.location.href="${ctx}/handler/goAdd.do";
	}

	function updatePROCEDUREMANAGEMENT(id){
		window.location.href="${ctx}/handler/goEdit.do?HANDLER_ID="+id;
	}
		...
		$(document).on("click",".equi_sc",function(){
			...
			parent.layer.open({
				...
				yes:function(index,layero){
					var PROCEDUREMANAGEMENT_ID = $(".lins_table tbody tr").eq(a).find("input[name='HANDLER_ID']").val();
					$.ajax({
						type: "POST",
						url: "${ctx}/handler/delete",
						data: {
							HANDLER_ID: PROCEDUREMANAGEMENT_ID
						},
						success: function(data) {
			...
			if(is==0){//当没有选中的时候
				...
				parent.layer.open({
					...
					yes:function(index,layero){
						...
						$.ajax({
							type: "POST",
							url: '${ctx}/handler/deleteAll.do?tm='+new Date().getTime(),
					    	data: {DATA_IDS:str},
							dataType:'json',
...
function upexcel(obj){
    ...
    }else if(!/\.(?:xls|xlsx)$/.test(f))
    {
        ...
        //上传
        $.ajaxFileUpload({
            url:"${ctx}/handler/readexcel",//需要链接到服务器地址   
            secureuri:false,
		...
		function toExcel(){
		window.location.href='<%=basePath%>handler/excel.do';
	}
