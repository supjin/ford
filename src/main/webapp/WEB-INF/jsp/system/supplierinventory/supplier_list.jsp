    ...
    <div class="oiswen_titop">
        <div class="oiswen_titople">
            ...
            <div class="oiswen_titopleb">
                <a href="javascript:;"><span class="daohangbefor glyphicon glyphicon-home"></span>HOME</a><span>/</span><a href="javascript:;">系统工具</a><span>/</span><a href="javascript:;">供应商管理</a>
            </div>
        </div>
    </div>
    <form action="${ctx }/supplierinventory/list" name="souForm" id="souForm">
	<!--主体--->
	<div class="content">
		<!--导出-->
	    ...
	    <table cellpadding="0" cellspacing="0" class="lins_table">
			...
			<c:forEach items="${varList}" var="var" varStatus="vs" >
	    		<tr>
	    			<td><input class="pull-right" type="checkbox" name='ids' value="${var.SUPPLIERINVENTORY_ID}" />
	    			<input type="hidden" name="SUPPLIERINVENTORY_ID" value="${var.SUPPLIERINVENTORY_ID }">
	    			</td>
	    			<td>${vs.index+1}</td>
					<td>${var.CODE}</td>
					<td>${var.NAME}</td>
					...
					<td>${var.LINKPHONE }</td>
					<%-- <td>${var.SUPPLIER}</td> --%>
	    			<td><div class="cuozuo_box clearfix"><a href="${ctx }/supplierinventory/goaddAndEdit?SUPPLIERINVENTORY_ID=${var.SUPPLIERINVENTORY_ID}" class="equi_bianji"></a><a href="javascript:;" class="equi_sc"></a></div></td>
	    		</tr>
	    		</c:forEach>
			</c:when>
			<c:otherwise>
	    ...
	    <!---添加删除--->
	    <div class="equi_btn">
	    	<a href="${ctx }/supplierinventory/goaddAndEdit">添加+</a>
...
<script>
		...
		$(document).on("click",".equi_sc",function(){
			...
			parent.layer.open({
				...
				yes:function(index,layero){
					var SUPPLIERINVENTORY_ID = $(".lins_table tbody tr").eq(a).find("input[name='SUPPLIERINVENTORY_ID']").val();
					$.ajax({
						type: "POST",
						url: "${ctx}/supplierinventory/delete",
						data: {
			...
			if(is==0){//当没有选中的时候
				...
				parent.layer.open({
					...
					yes:function(index,layero){
						$.ajax({
							type: "POST",
							url: '${ctx}/supplierinventory/deleteAll.do?tm='+new Date().getTime(),
					    	data: {DATA_IDS:str},
							dataType:'json',
    ...
    }else if(!/\.(?:xls|xlsx)$/.test(f))
    {
        ...
        //上传
        $.ajaxFileUpload({
            url:"${ctx}/supplierinventory/readexcel",//需要链接到服务器地址   
            secureuri:false,
