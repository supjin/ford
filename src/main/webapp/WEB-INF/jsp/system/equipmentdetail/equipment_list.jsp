    ...
    <div class="oiswen_titop">
        <div class="oiswen_titople">
            ...
            <div class="oiswen_titopleb">
                <a href="javascript:;"><span class="daohangbefor glyphicon glyphicon-home"></span>HOME</a><span>/</span><a href="javascript:;">系统工具</a><span>/</span><a href="javascript:;">设备管理</a>
            </div>
        </div>
    </div>
    <form action="${ctx }/equipmentdetail/list" name="souForm" id="souForm">
	<!--主体--->
	<div class="content">
		<!--导出-->
		<div class="dao_box clearfix">
			<input type="text" placeholder="名称、编号"/>
			<a href="${ctx}/equipmentdetail/excel">导出</a>
	    ...
	    <table cellpadding="0" cellspacing="0" class="lins_table">
	    	<thead>
	    		<tr>
	    			...
	    			<td style="width: 250px;">设备名称</td>
	    			<td style="width: 120px;">设备编号</td>
	    			<td style="width: 65px;">项目</td>
	    			<td style="width: 120px;">规格/型号</td>
	    			<td style="width: 65px;">功率</td>
	    			<td style="width: 120px;">品牌</td>
	    			<td style="width: 65px;">设备类别</td>
	    			<td style="width: 65px;">位置</td>
	    			<td style="width: 270px;">厂家</td>
	    			<td style="width: 110px;">到厂日期</td>
			...
			<c:forEach items="${varList}" var="var" varStatus="vs">
	    		<tr>
	    			<td><input class="pull-right" type="checkbox" name='ids' value="${var.EQUIPMENTDETAIL_ID}" />
	    			<input type="hidden" name="EQUIPMENTDETAIL_ID" value="${var.EQUIPMENTDETAIL_ID }">
	    			</td>
	    			<td>${vs.index+1}</td>
	    			<td>${var.NAME}</td>
					<td>${var.NUMBER}</td>
					...
					<td>${var.CATEGORY}</td>
					<td>${var.ADDRESS}</td>
	    			<td>${var.MANUFACTOR}</td>
					<td>${var.FACTORYDATE}</td>
					<td>${var.CONTACTS}</td>
	    			<td>${var.PHONE}</td>
	    			<td>${var.REMARKS}</td>
	    			<td style="width: 10%"><div class="cuozuo_box clearfix"><a href="${ctx}/equipmentdetail/goEdit?EQUIPMENTDETAIL_ID=${var.EQUIPMENTDETAIL_ID }" class="equi_bianji"></a><a href="javascript:;" class="equi_sc"></a></div></td>
	    		</tr>
			</c:forEach>
			</c:when>
			<c:otherwise>
	    ...
	    <!---添加删除--->
	    <div class="equi_btn">
	    	<a href="${ctx}/equipmentdetail/goAdd">添加+</a>
...
<script>
	...
	function upexcel(obj){
	    ...
	    }else if(!/\.(?:xls|xlsx)$/.test(f))
	    {
	        ...
	        //批量上传图片
	        $.ajaxFileUpload({
	            url:"${ctx}/equipmentdetail/readexcel",//需要链接到服务器地址   
	            secureuri:false,
	...
	$(function(){
		...
		$(document).on("click",".equi_sc",function(){
			...
			parent.layer.open({
				...
				btnAlign: 'c',//按钮居中
				yes:function(index,layero){
					var EQUIPMENTDETAIL_ID = $(".lins_table tbody tr").eq(a).find("input[name='EQUIPMENTDETAIL_ID']").val();
					$.ajax({
						type: "POST",
						url: "${ctx}/equipmentdetail/delete",
						data: {
		...
		$(".anniu_scbtn").click(function(){
			...
			parent.layer.open({
				...
				yes:function(index,layero){
					$.ajax({
						type: "POST",
						url: '${ctx}/equipmentdetail/deleteAll.do?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
						dataType:'json',
