		...
		<div class="oiswen_box oiswen_boxavite">
			...
			<div class="oiswen_titop">
				<div class="oiswen_titople">
					<div class="oiswen_titoplea">缺陷录入表</div>
					<div class="oiswen_titopleb">
						...
						<a href="${ctx }/imperfection_entry/list.do">缺陷录入</a>
					</div>
				</div>
			</div>
			<!----问题采集---->
			<div class="wenti_box">
				<form action="" name="imForm" id="imForm">
					<input type="hidden" name="IMPERFECTION_ENTRY_ID" id="IMPERFECTION_ENTRY_ID" value="${pd.IMPERFECTION_ENTRY_ID }">
					<div class="container">
						<div class="row">
							...
							<div class="col-sm-6 col-md-4 col-lg-4">
								<div class="inputboxa">
									<p>产品编号(外)</p>
									<div class="tadiv procok">${pd.CODEK }</div>
							...
							<div class="col-sm-6 col-md-4 col-lg-4">
								<div class="inputboxa">
									<p>操作者</p>
										<select class="chzn-select" name="DOUSER" id="DOUSER" data-placeholder="请选择" style="vertical-align:top; width: 100%; border: 1px solid #dedede;height: 29px;">
										<option></option>
										<c:forEach items="${handlerList }" var="var">
											<option value="${var.HANDLER_ID }" attid="${var.HANDLER_ID }" <c:if test="${pd.DOUSER eq var.HANDLER_ID }"> selected="selected"</c:if>>${var.USERNAME }</option>
										</c:forEach>
									</select>
								</div>
							</div>
					...
					<div class="acq_boxtj">
						<button type="button" class="jxbutton" onclick="window.location.href='${ctx}/imperfection_entry/list'">关闭</button>
						<button type="button" class="tibutton">提交</button>
						<c:if test="${(pd.IMPERFECTION_ENTRY_ID  == null) }">
						<button type="button" class="jxbutton" onclick="window.location.href='${ctx}/imperfection_entry/goAndandEdit'">继续添加</button>
						</c:if>
					</div>
				</form>
			</div>
		...
		<script>
			...
			$(function() {
				...
				$(document).on("click", ".tibutton", function() {
					...
					$.ajax({
						type: "POST",
						url: "${ctx}/imperfection_entry/addAndEdit",
