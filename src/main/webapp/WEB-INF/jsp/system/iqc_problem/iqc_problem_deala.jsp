...
<html>
	...
	<body style="background-color:#edeff5;margin: 15px;">
		...
		<input type="hidden" name="NEWSTATE" id="NEWSTATE" value="${pds.NEWSTATE }">
			<!---表---->
			<c:if test="${pds.TYPE eq 1 }">
			<div class="iqc_listbot">
			<form action="" id="codeForm" name="codeForm">
			<input type="hidden" name="TYPE" value="${pds.TYPE }">
			<input type="hidden" name="IQC_PROBLEM_ID" value="${pd.IQC_PROBLEM_ID }">
			<input type="hidden" name="FINISHTIMEA" id="FINISHTIMEA" value="${pd.FINISHTIMEA }">
			<input type="hidden" name="SENDMAN" id="SENDMAN" value="${pd.SENDMAN }">
				<div class="container" style="width: 100%;">
				    <div class="row">
				     <div class="col-sm-3">
				        ...
				        <div class="col-sm-3">
				        	<div class="iqc2div clearfix">
				        		<em>要求回复日期：</em><input class="layui-input test-item pw_layimg" type="text" id="demo1" name="REQUIREDATE" value="<fmt:formatDate value='${pd.REQUIREDATE }' pattern='yyyy-MM-dd HH:mm' />" onClick="laydate({istime: true, format: 'YYYY-MM-dd HH:mm'})"/>
				        	</div>
				        </div>
				        <div class="col-sm-3">
				        	<div class="iqc2div clearfix">
		...
		<c:if test="${pds.TYPE eq 2 }">
		<!---表---->
			<div class="iqc_listbot">
			<form action="" id="cofeForm" name="cofeForm">
			<input type="hidden" name="TYPE" value="${pds.TYPE }">
			<input type="hidden" name="IQC_PROBLEM_ID" value="${pd.IQC_PROBLEM_ID }">
			<input type="hidden" name="FINISHTIMEB" id="FINISHTIMEB" value="${pd.FINISHTIMEB }">
				<div class="container" style="width: 100%;">
				    <div class="row">
				    	<div class="col-sm-4">
				        	<div class="iqc2div clearfix">
				        		<em>完成日期：</em><input class="layui-input test-item pw_layimg" type="text" name="WANCDATE" value="${pd.WANCDATE }" id="demo1"/>
				        	</div>
				        </div>
				        <div class="col-sm-4">
		...
		<c:if test="${pds.TYPE eq 4 }">
		<!---表---->
			<div class="iqc_listbot">
			<form action="" id="coreForm" name="coreForm">
			<input type="hidden" name="TYPE" value="${pds.TYPE }">
			<input type="hidden" name="IQC_PROBLEM_ID" value="${pd.IQC_PROBLEM_ID }">
			<input type="hidden" name="FINISHTIMEB" id="FINISHTIMEB" value="${pd.FINISHTIMEB }">
				<div class="container" style="width: 100%;">
				    <div class="row">
				    	<div class="col-sm-4">
				        	<div class="iqc2div clearfix">
				        		<em>实际回复日期：</em><input class="layui-input test-item pw_layimg" type="text" id="demo1" name="SHIJIDATE"  value="<fmt:formatDate value='${pd.SHIJIDATE }' pattern='yyyy-MM-dd HH:mm' />" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})"/>
				        	</div>
				        </div>
				        <div class="col-sm-4">
				        	<div class="iqc2div clearfix">
				        		<em>完成日期：</em><input class="layui-input test-item pw_layimg" type="text" id="demo2" name="WANCDATE" value="<fmt:formatDate value='${pd.WANCDATE }'   pattern='yyyy-MM-dd HH:mm' />" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})"/>
				        	</div>
				        </div>
				    </div>
				    <div class="row">
		...
		<script>
			...
			$(document).on("click", ".iqcsubmito", function() {
				 $.ajax({
						type : "POST",
						...
						if(data.status==2){
							...
							if($("#NEWSTATE").val()==1){
								window.location.href='${ctx}/iqc_problem/list';	
							}else{
				...
				$(document).on("click", ".iqcsubmitf", function() {
				 $.ajax({
						type : "POST",
						url : "${ctx}/iqc_problem/addAndEdit",
			...
			$(document).on("click", ".iqcsubmitt", function() {
				//附件
				var accessory = "";
				var tis = $(this);
				tis.prev().find(".jinjiddi").find("a").each(function(index,el){
					var name = $(this).text();
