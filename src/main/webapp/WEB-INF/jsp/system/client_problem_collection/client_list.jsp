    ...
    <div class="oiswen_titop">
        <div class="oiswen_titople">
            ...
            <div class="oiswen_titopleb">
                <a href="javascript:;"><span class="daohangbefor glyphicon glyphicon-home"></span>HOME</a><span>/</span><a href="javascript:;">${pd.DALEIBIE }</a><span>/</span><a href="javascript:;">问题列表</a>
            </div>
        </div>
    </div>
    ...
    <div class="goncsbox clearfix">
        <div class="container list_divbox">
		    ...
		    <div class="row">
		    	<div class="col-sm-6">
		        	<div class="list_divtop">
		        		<span class="list_divtoptit">客户名称：</span>
		 ...
		 <!---按钮组--->
		 <div class="list_btns">
		 	<button type="button"  onclick='window.location.href="${ctx }/client_problem_collection/goaddAndEdit?TYPE=${pd.TYPE}"'>新增录入</button>
        	...
        	<table cellpadding="0" cellspacing="0" class="list_table">
        	    ...
        	    <c:choose>
        	    	...
        	    	<tr>
        	    		...
        	    		<c:if test="${var.ISOUTIME eq 1}"><a  class="list_bgdiv list_bgdiv2 dangsha" href="${ctx }/${var.PID}${var.CLIENT_PROBLEM_COLLECTION_ID}"><em>D${var.DANGQ }</em><span>${var.LABEL }</span></a></c:if>
        	    		<c:if test="${var.ISOUTIME eq 2}"><a  class="list_bgdiv list_bgdiv3 dangsha"  href="${ctx }/${var.PID}${var.CLIENT_PROBLEM_COLLECTION_ID}"><em>D${var.DANGQ }</em><span>${var.LABEL }</span></a></c:if>
        	    		<c:if test="${empty var.ISOUTIME}"><a  class="list_bgdiv list_bgdiv0 dangsha"  href="${ctx }/${var.PID}${var.CLIENT_PROBLEM_COLLECTION_ID}"><em>D${var.DANGQ }</em><span>${var.LABEL }</span></a></c:if>
        	    		...
        	    		<td>
        	    			<div class="addsbox list_addsbox clearfix">
        	    				<a class="liulanbtn pull-left" href="${ctx }/client_problem_collection/goBrows?CLIENT_PROBLEM_COLLECTION_ID=${var.CLIENT_PROBLEM_COLLECTION_ID}"><em></em><span>浏览</span></a>
        	    				<c:choose>
        	    				...
        	    				<c:otherwise>
        	    				<a class="bianjibtn pull-left" href="${ctx }/client_problem_collection/goaddAndEdit?TYPE=1&CLIENT_PROBLEM_COLLECTION_ID=${var.CLIENT_PROBLEM_COLLECTION_ID}"><em></em><span>编辑</span></a>
...
function Init(pageIndex) {
    ...
    $(document).on("click",".scbtn",function(){
    	...
    	parent.layer.open({
			...
			var CLIENT_PROBLEM_COLLECTION_ID = _this.next().val();
					$.ajax({
						type: "POST",
