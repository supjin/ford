...
<div class="oiswen_box oiswen_boxavite">
	...
	<!--主体--->
	<form action="${ctx}/material_namagement/list" name="souForm" id="souForm">
	<div class="content">
		<!--导出-->
	    ...
	    <table cellpadding="0" cellspacing="0" class="lins_table">
			...
			<c:forEach items="${varList}" var="var" varStatus="vs">
	    		<tr>
	    			<td><input class="pull-right" type="checkbox"  name='ids' value="${var.MATERIAL_NAMAGEMENT_ID}" />
	    				<input type="hidden" name="MATERIAL_NAMAGEMENT_ID" value="${var.MATERIAL_NAMAGEMENT_ID }">
	    			</td>
	    			<td>${vs.index+1}</td>
	    			<td>${var.PARTCODING }</td>
	    			<td>${var.PARTNAME }</td>
	    			<td>${var.NAME }</td>
	    			<td><div class="cuozuo_box clearfix"><a href="${ctx}/material_namagement/goAdd?MATERIAL_NAMAGEMENT_ID=${var.MATERIAL_NAMAGEMENT_ID}" class="equi_bianji"></a><a href="javascript:;" class="equi_sc"></a></div></td>
	    		</tr>
	    		</c:forEach>
				</c:when>
				<c:otherwise>
...
<script>
		...
		$(document).on("click",".equi_sc",function(){
			...
			parent.layer.open({
				...
				yes:function(index,layero){
					var MATERIAL_NAMAGEMENT_ID = $(".lins_table tbody tr").eq(a).find("input[name='MATERIAL_NAMAGEMENT_ID']").val();
					$.ajax({
						type: "POST",
						url: "${ctx}/material_namagement/delete",
						data: {
			...
			if(is==0){//当没有选中的时候
				...
				parent.layer.open({
					...
					yes:function(index,layero){
						$.ajax({
							type: "POST",
							url: '${ctx}/material_namagement/deleteAll.do?tm='+new Date().getTime(),
					    	data: {DATA_IDS:str},
							dataType:'json',
    ...
    }else if(!/\.(?:xls|xlsx)$/.test(f))
    {
        ...
        //上传
        $.ajaxFileUpload({
            url:"${ctx}/material_namagement/readexcel",//需要链接到服务器地址   
            secureuri:false,
